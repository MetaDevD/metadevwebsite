...
export default function App() {
  return (
    <div>
      <Header />
      <Canvas dpr={[1, 2]} shadows camera={{ position: [0, 0, 10], near: 0.5, far: 1000 }}>
        <ambientLight intensity={0.03} />
        <fog attach="fog" args={['#274ad2', 5, 18]} />
        <spotLight angle={0.14} color="#274ad2" penumbra={1} position={[25, 50, -20]} shadow-mapSize={[2048, 2048]} shadow-bias={-0.0001} castShadow />
...
function LittlestTokyo({ ...props }) {
  ...
  useFrame((state, delta) => {
    ...
    state.camera.position.set(Math.sin(offset) * -10, Math.atan(offset * Math.PI * 2) * 5, 8)
